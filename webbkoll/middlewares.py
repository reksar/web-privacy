...
class DownloaderRetryResults(RetryMiddleware):
    ...
    *status* resourse "https://webbkoll.dataskydd.net/en/status?id=<...>".
    ...
    It takes some time (10-20 sec) to check the URL, and while the check is
    incomplete, the Webbkoll *status* will respond with HTTP 200.

    Once the check is complete, you will get 302 redirect to *results*
    "https://webbkoll.dataskydd.net/en/results?url=<...>". You may get 302
    redirect immediately if *results* is cached.

    Retrying prevents the spider from parsing the *status* page instead of
    *results*.
    """
